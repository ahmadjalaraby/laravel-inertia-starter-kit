  ...
  <header :class="navbarTypeClass()">
    <div
      :class="`app-header md:px-6 px-[15px]  dark:bg-slate-800 shadow-base dark:shadow-base3 bg-white ${borderSwicthClass()} ${
        useThemeSettingsStore().navbarColor
      }
      ${
        useThemeSettingsStore().menuLayout === 'horizontal' && window.width > 1280
          ? 'py-1'
          : 'md:py-6 py-3'
      }
      `"
    >
      <div class="flex justify-between items-center h-full">
        <div
          v-if="useThemeSettingsStore().menuLayout === 'vertical'"
        ...
        >
          <button
            ...
            v-if="useThemeSettingsStore().sidebarCollasp && window.width > 1280"
            @click="useThemeSettingsStore().sidebarCollasp = false"
          >
            <Icon
              icon="akar-icons:arrow-right"
              v-if="!useThemeSettingsStore().direction"
            />
            <Icon
        ...
        <div
          v-if="useThemeSettingsStore().menuLayout === 'horizontal'"
...
export default {
  ...
  methods: {
    ...
    navbarTypeClass() {
      switch (useThemeSettingsStore().navbarType) {
        case "floating":
          return "floating";
        case "sticky":
    ...
    borderSwicthClass() {
      if (
        ...
        useThemeSettingsStore().navbarType !== "floating"
      ) {
